...
app.post("/generate", async (req, res) => {
    try {
        ...
        prompt = `${pages} these are the total pages needed with theme ${theme} navbar with pagenames,homepage as index.html give other outputs ${pagename}.html ${pagename}.css ${pagename}.js for ONLY ${pagename} page and routes.txt for routing names""as a json response with key as filename and value as content"". GENERATE  flask if absolutely necessary. Use routes assuming that all HTML pages are located in the same folder. DON'T GIVE ANY EXTRA OUTPUT THAN SPECIFIED. USE ${data} for routing. INCLUDE LINKING of ${filename} assuming it is in the folder. TOPIC: ${prompt} use images from stable diffution`;
